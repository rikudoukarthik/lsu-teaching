...
data_means <- data_full %>% 
  ...
  reframe(mean = mean(points),
          sd = round(sd(points), 3),
          se = round(sd/sqrt(n()), 3)) %>% 
  mutate(mean_label = paste("mean = ", mean),
         sd_label = paste("sd = ", sd),
         se_label = paste("se = ", se))
...
ggplot(data_full) +
  ...
  geom_text(data = data_means, mapping = aes(x = venue, y = mean + 1.75, label = mean_label),
            size = 4, fontface = "bold") +
  geom_text(data = data_means, mapping = aes(x = venue, y = mean + 1.25, label = sd_label),
            size = 4, fontface = "bold") 
  # geom_text(data = data_means, mapping = aes(x = venue, y = mean + 0.75, label = se_label),
  #           size = 4, fontface = "bold") +
  # geom_errorbar(data = data_means, aes(ymin = mean - 1.96*se, ymax = mean + 1.96*se, x = venue),
  #               width = 0.3)
